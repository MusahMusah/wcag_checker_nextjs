...
const Page = () => {
  ...
  const [analytics, setAnalytics] = useState<IWCAGData | null>(null);

  useEffect(() => {
    if (typeof window !== "undefined") {
      setAnalytics(LocalStorageHelper.get<IWCAGData>("wcag_analysis"));
    }
  }, []);
  ...
  return (
    <div className="mx-auto my-5 max-w-4xl px-4 py-2">
      ...
      >
        <h2 id="wcag-metrics" className="sr-only">
          WCAG Analysis Metrics
        </h2>
        ...
        <div
          ...
          className="flex w-full cursor-pointer items-center rounded-lg border bg-white p-6 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
          tabIndex={0}
          onKeyDown={(e) => {
            if (e.key === "Enter" || e.key === " ")
              alert(`Accessibility Score: ${analytics.accessibility_score}%`);
          }}
        >
        ...
        <div
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter" || e.key === " ")
              alert(`Detected Issues: ${analytics.issues.length}`);
          }}
        >
          <div className="flex flex-1 flex-col gap-1">
            <div
      ...
      <section className="my-5" aria-labelledby="issue-highlights">
        <h2
          id="issue-highlights"
          className="text-left text-2xl font-bold leading-8 text-slate-800"
        >
          Visual Highlights of Detected Issues
        </h2>
        <DetectedIssues issues={analytics.issues} />
      </section>
    </div>
  );
};
